...
internal object ChatPrompt {
  ...
  private const val TOOL_POLICY =
    "アプリ内データを確認する必要があり、適切なツールが利用可能なら実際にツールを呼び出してください。" +
      ...
      "「最近」「最新」「直近」「新しい順」はキーワード検索語ではなく取得順の意図として扱い、専用の最近取得ツールがあれば引数なしで利用してください。" +
      "検索ツールの結果が0件、または質問に答える根拠として明らかに不足する場合は、同じ検索語を繰り返さず、より短い語、同義語、固有名詞など具体的な検索語へ言い換えて再検索してください。" +
      "同じ情報要求に対する検索語の言い換えは最大2回までとし、それでも根拠が得られなければ見つからなかったことを回答してください。" +
      "質問に複数の情報要求が含まれる場合は、1つの曖昧な検索へまとめず、必要な情報源ごとに適切なツールを呼び出してください。" +
      "候補検索ツールと詳細取得ツールがある場合は、まず候補を検索し、回答根拠として必要な候補だけ詳細を取得してください。候補すべての詳細を取得しないでください。" +
