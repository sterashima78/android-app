...
import dev.terashima.yomitorirss.feature.task.TaskReader
...
class DefaultCalendarRepository(
  context: Context,
  private val taskReader: TaskReader,
  ...
  override suspend fun events(
    ...
    buildList {
      ...
      addAll(taskCalendarEvents(taskReader.listTasks(), fromInclusive, untilExclusive))

      val workout = workoutReader.load()
      addAll(
...
private class AndroidCalendarEventSource(
  ...
  fun events(
    ...
    cursor.use { rows ->
      ...
      val colorIndex = rows.getColumnIndexOrThrow(CalendarContract.Instances.DISPLAY_COLOR)
