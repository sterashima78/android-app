...
internal fun classifyKnowledgeProviderFailure(error: ChatGptProviderException): KnowledgeCloudInferenceException = when (error.kind) {
  ...
  ChatGptProviderFailureKind.WEB_TARGET_NOT_OPENED, ChatGptProviderFailureKind.UNKNOWN -> KnowledgeCloudInferenceException(KnowledgeCloudFailureKind.UNKNOWN, false, "ChatGPT / Codex のWiki生成に失敗しました")
}

private fun sanitizeUnknownKnowledgeAdapterMessage(message: String?): String = if (message.orEmpty().contains("利用モデルを選択", true)) "ChatGPT / Codex の利用モデルを選択してください" else "ChatGPT / Codex のWiki生成に失敗しました"
