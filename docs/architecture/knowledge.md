...
自動Wikiでは、タグを単に付与されているだけでは独立トピックへ昇格させない。同じ正規化タグを持つ要約済み資料が3件以上ある場合だけ、そのタグをKnowledgeトピックとして扱う。記事固有の語や一度しか現れないAI生成タグが、そのまま1資料だけのWikiページを量産することを避けるためである。

自動Wikiのトピック候補は次の優先順位で決める。

1. 3件以上の資料で使われるタグがある資料は、その昇格済みタグ単位
2. 昇格済みタグがなく通常ブックマークフォルダがある資料は、フォルダ単位
3. どちらもない資料は、提供元単位

同一資料が複数の昇格済みタグを持つ場合は複数トピックの根拠になり得る。昇格済みタグと3件未満の細分化タグを同時に持つ場合は、昇格済みタグだけをトピック化し、フォルダや提供元へ重複フォールバックしない。3件未満のタグ自体はBookmarkの検索・分類メタデータとして保持し、Knowledgeの自動Wikiトピック判定からだけ除外する。

トピックIDは種別と正規化キーから決定論的に生成する。閾値未満になって現在のトピック集合から消えた自動生成ページは、通常のobsolete page整理で削除対象になる。ユーザーが編集した `editor_managed = true` ページは従来どおり自動削除しない。
...
- [ADR-0230](../adr/0230-promote-knowledge-tag-topics-by-evidence-count.md)
