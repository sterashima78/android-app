...
- 一覧には「すべて / 続き / 保存済み / 視聴済み / 設定」を用意し、sourceを「すべて / SMB / Web / サービス」で絞り込める。「サービス」は購読型Providerが取得した動画を表す。
...
- 再生画面を開いている間は画面の自動消灯を抑止する。プレイヤー左半分をダブルタップすると15秒戻し、右半分をダブルタップすると15秒送りを行う。
