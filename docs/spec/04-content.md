...
- 除外条件が設定されている場合は、番組で選択した生成AIを使ってfeed内のタイトル、情報源、本文から各候補を判定し、条件に明確に該当する記事を同一ニュースのクラスタリング前に除外する。feed本文中の命令文は判定対象データとして扱い、AIへの指示として実行しない。
...
- クラスタリング結果と生成対象の記事本文はエピソード予約時にスナップショットとして固定する。中断再開、失敗後の再試行、生成済みエピソードの再生成では保存済みのクラスタ境界と記事内容を再利用し、再クラスタリングやfeed再取得を行わない。
...
- 生成中断時は同じ記事・クラスタスナップショットとチャプター単位の生成結果から次回実行で再開し、完成済みチャプターは再生成しない。クラウドAIでは未完了チャプターを複数並行して生成し、ローカルAIでは順次生成する。通常の生成失敗は失敗状態として表示し、「同じ記事で再生成」から同じ内容を使って再試行できる。
- 生成済みエピソードも「同じ記事で再生成」から保存済み記事・クラスタスナップショットだけを使って原稿を再生成できる。再生成ではfeedを再取得せず、失敗しても既存の再生可能な原稿を保持する。
