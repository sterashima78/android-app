...
- 未読一覧では通常の左スワイプを「既読」、深い左スワイプを「除外参考」とする。「除外参考」は記事を既読にしたうえで、そのタイトルと直前の推薦評価を今後の除外条件改善用feedbackとして保存する。右スワイプの「ブックマーク」と深い右スワイプの「あとで読む」は従来どおり維持する。
...
- 統合ビューの未読RSS記事にも同じ推薦評価を表示し、深い左スワイプから同じ「除外参考」操作を利用できる。統合ビュー独自の推薦状態や学習処理は持たず、RSSが所有する評価状態と操作を投影する。
