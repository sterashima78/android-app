...
- 手動または設定した定刻に、選択したPodcast sourceのfeedを取得し、その番組でまだ生成済みでも除外済みでもないentryから音声番組用の日本語原稿を生成する。RSS reader上の未読 / 既読状態は候補判定に利用しない。
...
- 生成中断時は同じ記事・クラスタスナップショットとチャプター単位の生成結果から次回実行で再開し、完成済みチャプターは再生成しない。クラウドAIでは未完了チャプターを複数並行して生成し、ローカルAIでは順次生成する。
...
- AIタスク一覧の進捗は記事数ではなく完了ニュースチャプター数 / 全ニュースチャプター数で表示する。
