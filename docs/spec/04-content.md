...
- 番組ごとに利用するPodcast source、生成AI、1回に使う最大ニュース数、毎日の自動生成時刻を設定できる。生成AIはローカルAIまたはクラウドAIを明示選択し、自動fallbackは行わない。
...
- Podcast source、番組とエピソード、生成に利用した記事・クラスタスナップショット、番組別の記事消費状態、生成原稿、定刻設定はdurable user stateとして保存し、通常のdatabase snapshot backup対象とする。削除済みエピソードでは生成原稿と記事スナップショットを消去し、entry再利用を防ぐ最小限のepisode参照と消費状態だけを保持する。
