...
- feedから取得した候補は、Unicode表記、前後・連続空白、大文字小文字の差を正規化したタイトルが一致する場合は同じニュースとして1件だけ残し、重複した記事を同じ生成候補へ含めない。語句の類似度による推測的な重複判定は行わない。
...
- 新しく生成する各チャプターには、元記事のタイトルをそのまま読み上げる代わりに、入力記事のタイトルと本文だけを根拠にした短い日本語の読み上げ見出しを生成する。外国語タイトルは自然な日本語へ翻訳・言い換え、入力にない事実は追加しない。読み上げ時はこの日本語見出しを1回だけ読み、本文内に同じ見出しや「タイトル」「見出し」といったラベルを重ねて読み上げない。
