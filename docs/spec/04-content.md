...
- RSS設定では「読みたい必要性が低い記事」の条件を自然文で設定でき、推薦評価と除外参考からの条件学習の実行先を端末内AIまたはクラウドAIから明示選択できる。既定は端末内AIとし、自動fallbackは行わない。条件がある場合、未読記事のタイトルだけを選択したAIの構造化tool callへ入力し、評価可能な記事だけを1〜10の推薦スコアとして表示する。1は除外条件への強い一致、10は正常に評価した結果として除外不要であることを表す。
- 推薦条件が有効で、まだ現在revisionの評価結果がない未読記事は「推薦: 評価待ち」と表示する。評価完了後は数値または未評価理由へ置き換える。タイトルだけでは条件との関係を判断できない記事は数値を付けず「タイトルだけでは判断できない」と表示する。推論またはtool call検証に失敗した記事も数値を付けず、判定失敗として区別する。除外条件が空の場合はスコアリング自体を行わない。
- 未読一覧では通常の左スワイプを「既読」、深い左スワイプを「除外参考」とする。通常左は短めの操作で確定し、除外参考はカード幅に対して明確に深くスワイプした場合だけ選択される。通常左から深い左へ切り替わる境界では表示と触覚フィードバックで操作の変化を伝える。「除外参考」は記事を既読にしたうえで、そのタイトルと直前の推薦評価を今後の除外条件改善用feedbackとして保存する。右スワイプの「ブックマーク」と深い右スワイプの「あとで読む」は従来どおり維持する。
- 手動入力した除外条件と、除外参考からAIが更新する学習条件は別々に保存し、判定時だけ結合する。AIは手動条件を書き換えない。連続した除外参考は最後の追加から約30秒待ってまとめて学習し、学習失敗時は既存条件と未処理feedbackを保持する。
- 推薦評価は判定に利用した条件revisionとともに保存する。実行先を変更した場合もrevisionを進め、現在未読の記事を新しい実行先で再評価する。RSS更新で取得した未読記事のうち現在revisionで未評価・古い評価・直前の推論失敗に該当する記事は、記事単位のバックグラウンドキューへ追加して順次評価する。条件が変わった場合も現在未読の記事を同じキューへ再投入する。評価処理は画面を閉じても継続し、端末内AI選択時は各記事の間で他の端末内AIタスクへ実行機会を渡す。クラウドAI選択時はネットワーク接続を必要とし、クラウドAIの一時停止設定に従う。
- 統合ビューの未読RSS記事にも同じ推薦評価を表示し、RSS未読一覧と同じ通常左 / 深い左の操作距離と切替フィードバックで「既読」と「除外参考」を利用できる。統合ビュー独自の推薦状態や学習処理は持たず、RSSが所有する評価状態と操作を投影する。

## 4.2 ブックマークとあとで読む

- コンテンツをブックマークとして保存できる。
- タグとフォルダで整理できる。
- ブックマーク一覧は既定でブックマーク日時の新しい順に表示し、ブックマーク日時 / 記事公開日時と新しい順 / 古い順を切り替えられる。選択した日時基準は一覧の日付見出しと各項目の日時表示にも利用する。
- 「あとで読む」は Curation Context が所有する system folder として扱う。
- RSS の「あとで読む」一覧の日付見出しと各項目の日時表示にはブックマーク日時を利用する。
...
- レビューの状態変更は固定ボタンの「保留」「未分類へ」「削除」で行い、「未分類へ」「削除」の後は自動的に次の記事へ進む。これらの変更は下部の固定操作領域と重ならない位置に短時間表示される Snackbar の「元に戻す」から復元でき、操作しなければ Snackbar は自動的に閉じる。
...
- 番組ごとに利用するPodcast source、生成AI、1回に使う最大ニュース数、毎日の自動生成時刻、聞かなくてよいニュースの除外条件を設定できる。除外条件は自然文で入力し、空欄なら内容による除外を行わない。生成AIはローカルAIまたはクラウドAIを明示選択し、自動fallbackは行わない。
- 手動または設定した定刻に、選択したPodcast sourceのfeedを取得し、その番組でまだ生成済みでも除外済みでもないentryから音声番組用の日本語原稿を生成する。RSS reader上の未読 / 既読状態は候補判定に利用しない。画面から開始した生成もdurableなbackground workとして実行し、画面を離れたり端末をロックしたりしてもUIのlifecycleに依存せず継続する。
...
- 除外判定の推論または構造化出力の検証に失敗した場合は記事を欠落させず、その回は全候補を残して通常のクラスタリングへ進む。除外されたentryは番組単位で除外済みとして記録し、次回以降は再判定しない。除外条件を後から変更しても過去の除外済みentryは自動復活させない。
...
- クラスタリング結果と生成対象の記事本文はエピソード予約時にスナップショットとして固定する。生成中断からの自動再開は保存済みのクラスタ境界と記事内容を再利用する。利用者が明示的に作り直す場合はfeedを再取得せず、そのエピソードに保存済みの記事本文へ現在の除外条件を再適用して再クラスタリングする。
- 番組ごとに一度生成対象として予約したentryは、同一ニュースに含まれる全記事を含めて消費済みとし、次回の新規エピソードでは再利用しない。Podcast生成・再生によってRSS reader側の記事の既読 / 未読状態は変更しない。
- 1回の最大ニュース数を超える未消費クラスタは生成時点の記事スナップショットとクラスタ境界を後続エピソード用に保存し、フィード更新で元entryが消えても後続生成に利用できる。
- 生成待ちのエピソードは通常一覧から明示的に生成開始できる。複数件ある場合は最古の生成待ちを先に開始し、番組上部の生成操作も生成待ちがあれば新しいfeed取得よりその処理を優先する。
- 生成中断時は同じ記事・クラスタスナップショットとチャプター単位の生成結果から次回実行で再開し、完成済みチャプターは再生成しない。クラウドAIでは未完了チャプターを複数並行して生成し、ローカルAIでは順次生成する。クラウドAIの一時的な通信失敗、rate limit、timeout、server error等でretryableと判定されたチャプターは、短いbackoffを挟んで最大3回自動再試行し、再試行を使い切った場合だけ生成失敗として確定する。
- 生成失敗または生成済みのエピソードは「現在の条件で作り直す」から再構築できる。現在そのエピソードに保存されている記事へ現在の除外条件を適用し、残った記事を再クラスタリングして同じエピソードIDで新しい原稿を生成する。feedは再取得せず、新しい記事は追加しない。作り直しで除外された記事はそのエピソードのsnapshotから外れ、後から条件を緩めても自動復帰しない。全記事が現在の条件で除外される場合は作り直しを開始せず既存内容を保持する。作り直し開始後は旧原稿を破棄し、途中で失敗した場合は生成失敗状態として扱う。
- 新しく生成する原稿では1ニュースクラスタを1チャプターとして扱う。再生時にはチャプター一覧、現在チャプター、そのニュースに含まれるすべての元記事をフルスクリーンの再生詳細画面へ表示する。複数記事がまとまったチャプターでは関連記事件数を表示し、各記事のfeed entry URLを個別に開ける。
- 再生詳細画面では再生操作を常に画面内へ表示し、チャプター / 関連記事一覧だけを独立してスクロールできる。
- 新しく生成する各チャプターには、元記事のタイトルをそのまま読み上げる代わりに、クラスタ内の記事タイトルと本文だけを根拠にした短い日本語の読み上げ見出しを生成する。外国語タイトルは自然な日本語へ翻訳・言い換え、入力にない事実は追加しない。読み上げ時はこの日本語見出しを1回だけ読み、本文内に同じ見出しや「タイトル」「見出し」といったラベルを重ねて読み上げない。
...
- 構造化されたチャプターは1ニュースを1つの音声再生itemとして扱う。「前 / 次」は前後チャプターへ移動し、再生 / 一時停止、15秒戻し、30秒送り、再生速度変更はRSS要約の音声再生と同じ共通操作を利用する。
...
- 一覧には「未視聴 / 保存済み / 視聴済み / 設定」を用意し、sourceを「すべて / SMB / Web / サービス」で絞り込める。「サービス」は購読型Providerが取得した動画を表す。
- 「未視聴」には保存済みではなく、視聴済みでもない動画を表示する。再生途中の未保存動画も引き続き「未視聴」に表示する。SMB由来とWeb URL由来の動画は常に保存済みのため「未視聴」には表示しない。
