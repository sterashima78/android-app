...
- 生成失敗または生成済みのエピソードは「現在の条件で作り直す」から再構築できる。保存済み記事に現在の除外条件を適用し、残った記事を再クラスタリングして同じエピソードIDで新しい原稿を生成する。feedは再取得せず、新しい記事は追加しない。全記事が現在の条件で除外される場合は作り直しを開始せず既存内容を保持する。作り直し開始後は旧原稿を破棄し、途中で失敗した場合は生成失敗状態として扱う。
