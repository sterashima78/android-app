...
- タイトルだけでは条件との関係を判断できない記事は数値を付けず「タイトルだけでは判断できない」と表示する。推論またはtool call検証に失敗した記事も数値を付けず、判定失敗として区別する。除外条件が空の場合はスコアリング自体を行わない。
...
- 推薦評価は判定に利用した条件revisionとともに保存する。条件が変わった後の古い評価は現在の結果として表示せず、現在未読の記事だけを新しい条件で順次再評価する。
