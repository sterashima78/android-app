...
- RSS設定では「読みたい必要性が低い記事」の条件を自然文で設定できる。条件がある場合、未読記事のタイトルだけを端末内AIの構造化tool callへ入力し、評価可能な記事だけを1〜10の推薦スコアとして表示する。1は除外条件への強い一致、10は正常に評価した結果として除外不要であることを表す。
...
- 推薦評価は判定に利用した条件revisionとともに保存する。RSS更新で取得した未読記事のうち現在revisionで未評価・古い評価・直前の推論失敗に該当する記事は、記事単位のバックグラウンドキューへ追加して順次評価する。条件が変わった場合も現在未読の記事を同じキューへ再投入する。評価処理は画面を閉じても継続し、各記事の間で他の端末内AIタスクへ実行機会を渡す。
