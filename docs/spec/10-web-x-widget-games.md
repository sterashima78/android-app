...
- X WebViewでは表示中のvideo要素について、再生準備状態、network状態、media error、再生状態、描画サイズ、source種別、親要素のレイアウト状態を端末上で診断表示し、診断結果をクリップボードへコピーできる。高さ0に潰れた動画領域は診断画面から一時的な表示補正を適用できる。
