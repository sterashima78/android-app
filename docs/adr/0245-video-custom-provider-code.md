# ADR-0245: 動画購読Providerでユーザー定義コードを実行できるようにする
...
Provider codeはユーザー自身が登録するtrusted configurationとして扱う。ただしsyntax error、runtime error、不正な戻り値、HTTP失敗、timeout、request上限超過は当該subscriptionのrefresh失敗として扱い、他Provider / subscriptionの更新を妨げない。
...
- application database versionを次versionへ進める。
- 既存組み込みProvider rowはfunction codeなしでそのまま維持する。
- migrationでは `video_providers` の既存row、subscription、provider itemを保持したままcustom Provider設定用column / constraintへ移行する。
