# ADR-0206: multi-route feature の navigation metadata を owning feature UI に置く
...
- multi-route feature の destination 追加・名称変更を owning feature UI に局所化できる。
- `AppNavigationSpec` が app-wide navigation capability / grouping に集中する。
- app chrome は feature-local mapping を再定義せず public feature contract を利用する。
- route identity の正本という ADR-0205 の説明と実装が一致する。

### Negative

- feature UI の public contract に navigation metadata function が増える。
...
- `AppNavigationSpecTest` で各 feature tab の route round-trip と全 registered route の title 解決を確認する。
