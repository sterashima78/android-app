# ADR-0178: Web Library custom extractor の Promise 監視を native watchdog で保証する
...
しかし実装上、10 秒の Promise deadline と polling は `WebView.evaluateJavascript` で start script を送信した後、その callback が返ってから開始されていた。page JavaScript が長時間 main thread を占有する場合や、navigation 等により evaluate callback が戻らない場合、custom execution は `RUNNING` のままになり、Promise の 10 秒上限ではなく rule の WebView 全体 timeout まで待つことになる。

この状態では WebView 全体 timeout を 60 秒へ延長しても custom result は確定せず、単に待ち時間だけが伸びる。ADR-0173 が意図した「停止した user-authored Promise を10秒で打ち切る」という保証を Android 側の callback availability に依存させない必要がある。
...
- Promise の10秒上限が native callback availability に依存しなくなる。
