...
コンテキスト長はLiteRT-LM `EngineConfig.maxNumTokens`へ渡す。Engine再利用キーとLiteRT-LM cache pathにもcontext tokensを含め、異なるKV cache容量のEngineやruntime cacheを混在させない。
...
Speculative decodingのExperimental flagはEngine初期化中だけ変更し、推論中にはグローバル状態を保持し続けない。
