...
2026-08-16のADR-0081で、固定8,192トークンの実行コンテキストは、端末実測に基づくAutoと4K/8K/16K/32Kの手動指定へ置き換えた。階層要約は固定値を持たず、`core:ai-runtime` が公開する現在の実効コンテキストへ追従する。
...
`core:ai-runtime` は現在の実効 `contextTokens` と汎用 `countTokens(text)` を技術能力として公開する。実行コンテキストの決定方法、端末メモリ計測、Auto選択、LiteRT-LM `EngineConfig.maxNumTokens` への反映はADR-0081に従う。

階層要約は4K/8K/16K/32Kのどれが選択されたかを判断せず、公開された実効 `contextTokens` を入力予算としてそのまま使う。これにより端末能力に余裕がある場合はチャンク数を減らし、余裕がない場合は小さいコンテキストで従来どおり階層要約できる。
...
- context: 選択中モデルの現在の実効 `contextTokens`
...
4. 各チャンクから、固有名詞、数値、日時、主張、結論、因果関係を優先した中間要約を作る。中間要約目標は実効コンテキストから計算し、最大600文字とする。
...
ADR-0081以降はruntime cache variantへ実効contextを含める。4K/8K/16K/32Kの変更でチャンク境界や推論回数が変わるため、異なる実効contextで生成した要約を同一runtime条件として再利用しない。
...
実効コンテキストを大きくするとKV cache由来の推論時メモリは増える。ADR-0081のAutoでは端末上のPSSと空きメモリを実測して安全余白を持つ最大値を選び、未計測時は8Kへフォールバックする。手動設定はこの安全判定を明示的に上書きする。
...
- ADR-0081: ローカルAIのコンテキスト長を端末実測で調整する
