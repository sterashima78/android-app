# ADR-0023: X カスタム CSS 設定はボトムシートで表示する
...
設定内容と保存方式は ADR-0008 および ADR-0022 で定義済みであり、今回の要件は保存モデルではなく表示コンテナの改善である。
...
- 下部から展開するため、設定画面から一時的な編集画面へ遷移する操作として認識しやすい。
...
- ADR-0008 の X WebView / CSS 注入方針は変更しない。
