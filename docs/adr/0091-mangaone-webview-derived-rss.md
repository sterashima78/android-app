...
さらに、数値話IDが古い話を指すURLでは、入力URLに付いている一覧ページ番号を除去した状態でWebViewを開くと、話一覧の初期化が遅延して20秒以内に `#chapterList` を取得できないケースが確認された。マンガワンの共有URLでは話一覧の表示状態として `type=chapter&sort_type=desc&page=...&limit=10` が付与されているため、フィード識別子と描画用URLを分離する必要がある。
...
WebViewは45秒でタイムアウトし、完了・失敗・キャンセルのいずれでも破棄する。ファイルアクセスとcontent URIアクセスを無効化し、mixed contentを拒否し、トップレベル遷移は `manga-one.com` / `www.manga-one.com` に限定する。JavaScript bridgeは追加しない。
...
`#chapterList` がまだ存在しない段階からページ末尾へスクロールし、遅延マウントを促す。`#chapterList` が現れた後も一覧領域を末尾へスクロールしながら500ms間隔で再取得する。同一の話一覧が複数回連続して得られた時点で確定し、遅延ロード中の一時的な一覧を保存しない。
