...
### 3. `#chapterList` の話行をアンカー要素に依存せず抽出する
...
話行またはその子孫・近傍祖先に同一作品の `/manga/{作品ID}/chapter/...` を含む `href` やdata属性等が存在する場合は、そのURLを記事の直接リンクとして使用する。直接URLをDOMから取得できない場合でもフィード生成自体は失敗させず、描画に使用した話一覧URLへ話タイトルと表示日から生成した安定したfragmentを付けた代替URLを記事URLとして使う。代替URLは直接その話を開く保証はないが、最新話一覧へ到達でき、記事ごとの重複判定キーを安定させられる。
...
`onPageFinished` はSPAの非同期DOM構築完了を示さないため、取得開始条件には使わない。`loadUrl` の直後から500ms間隔でJavaScriptを評価し、ページ末尾と話一覧領域をスクロールしながら `#chapterList` と話タイトル行を確認する。同一の話一覧が複数回連続して得られた時点で確定し、遅延ロード中の一時的な一覧を保存しない。

ポーリング結果は「document loading」「サービス側エラー表示」「chapter list未生成」「chapter row未生成」「ready」を内部状態として区別する。タイムアウト時は最後に観測した状態に応じて一般化したエラーメッセージを返す。実際の作品URL、作品ID、話ID、Cookie等をエラーメッセージやログへ埋め込まない。
...
- DOMに直接話URLが露出していない場合、記事タップは該当話ではなく最新話一覧へ遷移する。
- ETag / Last-Modifiedによる304最適化を利用できない。
- マンガワンのDOM構造やラベル表現が変わった場合はアダプターの修正が必要になる。
- Android WebView実装へ依存するため、純粋なJVM単体テストでは描画部分を直接再現できない。URL判定・フィード変換・User-Agent変換・代替URL生成は単体テストし、実DOM描画は実機確認が必要になる。
