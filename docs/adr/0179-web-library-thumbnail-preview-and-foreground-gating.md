...
また Web の画像配信元には、通常のブラウザと同様に参照元、画像向け Accept、ブラウザ User-Agent を要求するものがある。設定画面に preview を追加する場合、thumbnail URL だけを直接取得すると、metadata として URL が正しく取得・保存されていても画像ロードだけが失敗し得る。
