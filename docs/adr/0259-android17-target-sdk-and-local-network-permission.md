# ADR-0259: Android 17 / API 37 を target SDK baseline とする
...
- SMBは既存のSMB接続設定を持つLibraryを利用するときにlocal-network permissionを要求する。許可後は同じOS permissionを同期、reader、cover prefetch、metadata normalization、file operation等のLAN通信で利用する。
- permissionを拒否した場合、永続設定やcredentialを削除しない。ユーザーが後からOS設定または再要求で許可できる状態を維持する。
...
- SMB設定を持つLibrary routeでpermission未付与時に要求し、許可後に既存SMB機能が利用できることをAndroid 17実機で確認する。
...
- local-network permissionがない状態で実行されるbackground SMB workerは、OSのpermission failureを再試行ループへ変換せず、ユーザーによる許可取得を待つ状態として扱えるかを継続監視する。
