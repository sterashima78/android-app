...
フォルダの作成・予約・昇格は Bookmark feature が所有する。YouTube UI は `system_kind=youtube` のフォルダを BookmarkRepository から取得し、フォルダ指定付きの共有ブックマーク保存 API を利用する。YouTube domain / data には Bookmark 依存を追加しない。
...
- ADR-0003 の `UI -> 他 feature の Domain` を許容する依存ルールに従い、`youtube:ui` だけが Bookmark domain を利用する
