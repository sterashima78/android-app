...
表紙補完は Kindle インポート完了後に WorkManager で実行し、インポートの成否を外部サービスの状態に依存させない。ネットワーク接続を制約とし、通信障害は backoff 付きで再試行する。1回の Worker が扱う件数を制限し、継続分は次の Worker に連結する。

Open Library の未識別リクエスト制限を超えないよう検索リクエスト間隔は 1 秒を超える値とする。検索結果に含まれる CoverID を保存 URL に利用し、ISBN 等による Covers API の追加レート制限を避ける。ユーザー個人の連絡先を User-Agent に埋め込むことはしない。
...
- Open Library の公開レート制限に沿って低頻度で利用する
...
- 1秒超の間隔で処理するため大量蔵書の初回補完には時間がかかる
