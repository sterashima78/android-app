# ADR-0247: ニュースポッドキャストを独立Contextとして所有する
...
また、AI生成失敗後に番組IDだけで再実行すると、既にconsumedとして予約した記事とは別の候補を取り込み、失敗したepisodeの再生成という意味を失う。
...
### 入力は生成開始時点の未読記事かつfeed-carried contentに限定する

PodcastはContent-owned `ArticleRepository` から未読記事のidentityとmetadataを取得し、RSS-owned `RssFeedContentReader` からRSS / Atomフィードに含まれるtitle / bodyを取得する。

両者をfeed IDとsource identityで照合し、現在未読である記事だけを候補にする。
...
生成開始時に、番組内で未消費の候補から最大件数を選び、次を単一transactionで保存する。

- episode row
- episode article snapshot
- consumed article row

最大記事数はconsumed article除外後に適用する。上限を超えた未消費記事は後続episode候補として残す。

AI推論はtransaction完了後に行う。これにより生成中にRSS内容やreading stateが変わっても、episodeの入力を固定できる。

### 失敗は同じsnapshotで明示的に再生成し、中断は次回実行で再開する

AI推論が通常のエラーで失敗した場合はepisodeをFAILEDとして保存し、article snapshotとconsumed stateを保持する。

再生成はepisode IDを指定して既存snapshotを利用する。定刻Workerは通常エラー時に番組IDで自動retryしない。番組IDだけのretryでは別記事を予約できてしまうためである。

一方、Worker停止やcoroutine cancellation、プロセス終了など、生成処理が完了結果を確定できない中断ではepisodeをFAILEDへ変更せずGENERATINGのまま保持する。次回の番組生成は新しいfeed候補を取得する前に既存のGENERATING episodeを探し、その保存済みarticle snapshotから生成を再開する。これにより中断だけを理由に予約済み記事が取り残されたり、別記事へ進んだりしない。
...
生成promptは入力記事だけを根拠とし、日本語の音声合成向け連続文章を要求する。
...
### scheduleはPodcastが所有し、application compositionはwiringだけを行う

Podcast Dataのscheduler adapterが番組ごとの1日1回の定刻実行を登録・解除する。

番組作成・編集時の明示的なschedule同期では、現在時刻から新しい目標時刻までのinitial delayを再計算し、既存の周期workをcancel / re-enqueueして時刻変更を反映する。

application起動時のschedule reconciliationは既存の周期workを置き換えない。保存済みprogramのworkが存在しない場合だけ登録し、既存workはKEEPする。これによりbackground Worker起動のためにapplication processが生成された場合にも、そのWorker自身をstartup reconciliationでcancelしない。
...
- RSS記事を既読にしても過去episodeや生成済みscriptは保持される。
...
- 最大記事数を超えた未消費記事が後続episodeへ回ることをunit testする。
...
- promptがfeed title / bodyだけを根拠にする制約を持つことをunit testする。
- scheduleの次回delay計算をunit testする。
