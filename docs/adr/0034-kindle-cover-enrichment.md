# ADR-0034: Kindle 表紙を Open Library から明示的に補完する
...
Open Library は認証不要の公開 Search API と Covers API を提供する。ただし検索時にはタイトル、著者、ISBN などの書誌情報が外部サービスへ送信されるため、Kindle インポートの端末内処理とは明確に分離する必要がある。
...
- Open Library の検索・応答仕様変更には追従が必要になる
