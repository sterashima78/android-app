# ADR-0124: Android 14 を最小プラットフォーム基準とする
...
Android 17 / API 37 は compile SDK として利用できるが、`targetSdk` を 37 に上げることは Android 17 の target-specific behavior change を受け入れる別の互換性判断である。最小対応 OS の変更と target SDK の変更を同一の移行として扱わない。

## Decision

- アプリの `minSdk` を API 34 (Android 14) とする。
- `compileSdk` を API 37 とし、Android 17 SDK でコンパイルする。
- `targetSdk` は今回 API 36 のまま維持する。API 37 への変更は Android 17 の互換性検証を行う別変更で判断する。
...
- CI は API 37 platform を明示的に導入して quality/build を実行する。
...
### minSdk と targetSdk を同時に 37 へ上げる

一度に最新化できるが、インストール下限の判断と Android 17 の target-specific behavior change の検証が混在する。問題発生時の切り分けも難しくなるため採用しない。
