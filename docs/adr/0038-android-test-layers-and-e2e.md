...
### 2026-09-19 local test runtime update
...
一方、`src/androidTest` は AndroidX Test の `AndroidJUnitRunner`、Compose UI Test の JUnit 4 integration、UI Automator を利用する現在の公式構成を維持する。したがって JUnit 4 compatibility は instrumented test と既存 local test の移行期間に限定して残す。
