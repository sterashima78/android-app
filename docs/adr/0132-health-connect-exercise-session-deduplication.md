...
- 詳細付き session に `ExerciseSegment` があり、別提供元の standalone session に segment がない場合は、session 全体だけでなく各 segment と standalone session の時間帯にも同じ 80% / 55% の判定を適用する。ただし部分一致では、Data 層で変換した運動表示種別が segment と standalone session で一致することも必須とする。いずれかの segment と種別・時間帯が強く一致すれば、その standalone session は詳細付き session の一部を別提供元が重複記録したものとして統合する。
...
- session 全体同士の近似重複判定では exercise type の一致を必須としない。複数アプリが同じ実運動を `OTHER_WORKOUT` と `WALKING` など別 type で記録するケースを吸収するためである。
...
- segment 部分一致では運動表示種別の一致も要求するため、同時間帯にある別種別の運動を過度に統合しにくい。
...
- 異なる提供元が、詳細付き session の同種別 segment と時間帯・長さが近い別の実運動を同時に記録した場合は誤って統合する可能性が残る。
...
- 長い詳細付き session の同種別 segment とほぼ一致する、別提供元の短い standalone session が統合されることを unit test する。
- segment と十分に一致しない standalone session は保持されることを unit test する。
- 時間帯が一致しても segment と standalone session の運動表示種別が異なる場合は保持されることを unit test する。
