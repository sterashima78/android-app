...
`generate()` 開始時に main process は、その generation が利用する設定を1つの immutable snapshot として確定し Binder へ渡す。

snapshot に含めるのは次の inference execution metadata に限定する。

- selected model id
- selected model artifact revision
- backend
...
user が generation 実行中に model/backend/context 設定を変更しても、実行中 generation は開始時 snapshot を最後まで利用する。次の generation は main process から新しい snapshot を取得する。これにより token counting と generation の設定境界を generation 単位で明確にする。
...
- selected model revision も snapshot から child 専用 preference へ与え、model artifact validation は既存 `LocalModelManager` の規則を再利用する。
...
設定変更の実機確認では、generation 実行中に model/backend/context 設定を変更しても、実行中 generation が開始時 snapshot で完了し、次 generation から新設定が利用されることを確認する。child process 終了後に main process の model/backend/context preference が旧値へ戻らないことも確認する。
