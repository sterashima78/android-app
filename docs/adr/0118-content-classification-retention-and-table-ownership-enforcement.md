# ADR-0118: Content Classification・Retention・table ownership を明示的な境界へ移す
...
Article data は Summary の table 名・state schema を参照しない。

### 3. durable table ownership を manifest と CI で検査する

`config/architecture/table-ownership.tsv` に durable table と owner data module を記録する。
...
- `SummaryContentRetentionProtectionQueryTest`: Summary owner adapter が summary / queued / running のみを保護することを検証する。
