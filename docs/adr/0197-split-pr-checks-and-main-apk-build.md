...
- Superseded in part by: [ADR-0200](0200-remove-main-apk-commit-status.md) Decision 5
...
5. ADR-0093 で定めた signed release APK、APK signature verification、artifact upload は維持する。`apk/main` commit status は ADR-0200 により廃止する。
...
ADR-0200 により Decision 5 の `apk/main` commit status publication のみを廃止する。signed release APK の生成、署名検証、artifact upload は引き続き有効とする。
