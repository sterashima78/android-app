...
シリーズ作品では巻数を `title` から除くだけで失わないことを固定 instruction と Tool field description の双方で要求する。独立したシリーズ名表記がなくても巻数が明確なら、巻数を除いた作品タイトルを `seriesName` としてよい。既定 prompt でも漫画・ライトノベル・小説のシリーズ名と巻数を優先的に確認し、末尾の `08` のような1〜3桁数字も表紙と照合すべき巻数候補として扱う。

小型モデルがそれでも `seriesPosition` を省略した場合に備え、現在ファイル名が「作品名 + 末尾1〜3桁」で、数字を除いた文字列と AI が返した `title` が空白・記号を除いて一致する場合に限り、Library 側がその末尾数字を `seriesPosition` として補完する。1〜300だけを対象とし、既に AI が返したシリーズ情報は上書きしない。`seriesName` が未指定なら一致済みの `title` を使用する。これにより `架空冒険譚08.pdf` のような強いケースを補完しつつ、数字自体がタイトルの一部であるケースでは文字列一致条件により補完を避ける。
...
AI に SMB path や変更後ファイル名を自由生成させない。変更後ファイル名はアプリ側で `title` と `seriesPosition` から決定的に生成し、元拡張子を維持する。巻数がある場合は元ファイル名の表記に依存せず `title 第N巻.ext` を標準形とする。例えば `seriesPosition = 12` なら `架空冒険譚 第12巻.cbz` のように生成する。これにより `title` は巻数を含まない正規化書誌のまま保持しつつ、SMB 上の各巻のファイル名は衝突せず、巻数表記も統一できる。
...
- モデルが「作品名 + 末尾数字」から巻数を落とした場合も、タイトル一致を条件とする保守的な補完で代表的な欠落を回復できる。
...
- 末尾数字の補完はタイトル一致と1〜300の範囲へ限定するが、数字が巻数ではない特殊な命名では誤補完の可能性が残るため、レビューで修正・却下できることを前提とする。
