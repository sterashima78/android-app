...
一方、YouTube は「未読・あとで見る・保存済み・購読管理」を画面上部の横並びボタンで切り替えており、メールも主要なトリアージ表示を画面上部の `FilterChip` で切り替えていた。同じアプリ内で主要表示のナビゲーション位置が一致していなかった。
...
メール feature のトリアージ主要表示である次の4表示を画面下部タブへ配置する。
...
「あとで読む」は ADR-0008 が定義する Yomitori 独自のローカル状態であり、Gmail のスターとは別タブとして扱う。

アカウント切り替え、アカウント追加・削除、同期状態、検索は現在の表示に対する補助操作のため画面上部に残す。

メールスレッド詳細は単一コンテンツに集中する detail screen として扱い、詳細表示中は下部タブを表示しない。
...
- ADR-0008 が定義するメールの「未読・あとで読む・スター・アーカイブ」というトリアージ表示分離は維持し、本 ADR がその切り替え位置を定める
