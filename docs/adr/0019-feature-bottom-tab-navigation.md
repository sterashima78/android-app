...
一方、YouTube は「未読・あとで見る・保存済み・購読管理」を画面上部の横並びボタンで切り替えており、メールも「未読・スター・アーカイブ」を画面上部の `FilterChip` で切り替えていた。同じアプリ内で主要表示のナビゲーション位置が一致していなかった。
...
メール feature のトリアージ主要表示である次の3表示を画面下部タブへ移す。
...
- ADR-0008 が定義するメールの「未読・スター・アーカイブ」というトリアージ表示分離は維持し、本 ADR がその切り替え位置を定める
