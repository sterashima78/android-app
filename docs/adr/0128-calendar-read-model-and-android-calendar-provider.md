# ADR-0128: Calendar を複数 Context と Android Calendar Provider の read model として扱う

- Status: Accepted
- Date: 2026-08-19
- Refines: ADR-0103, ADR-0106, ADR-0116, ADR-0120, ADR-0122, ADR-0125, ADR-0126
...
### 5. Task / Workout は owner の narrow read capability 経由で読む

ADR-0125 の capability segregation に従い、`:feature:calendar:data` は full Repository ではなく次の read capability に依存する。

- `TaskReader.listTasks()` から `dueDate` のある Task を `DEADLINE` / all-day event へ投影
- `WorkoutReader.load()` から日付を持つ workout history / current result を `ACTIVITY` / all-day event へ投影

既存 `TaskRepository` / `WorkoutRepository` はそれぞれ read capability を継承する facade とし、Calendar は command capability を受け取らない。
...
- Calendar data は Task / Workout の command capability を持たない
...
- source read capability を期間 query に最適化していないため、Task / Workout は現状全件 load 後に Calendar data layer で期間 filter する
...
- provider result と要求日付範囲の overlap
