...
fun readTsv(path: String, minimumColumns: Int): List<List<String>> =
...
val tableOwners: Map<String, String> =
  readTsv("config/architecture/table-ownership.tsv", 2)
    .associate { columns -> columns[0].lowercase() to columns[1] }

val foreignTableAllowlist: Map<Pair<String, String>, String> =
  readTsv("config/architecture/foreign-table-access-allowlist.tsv", 3)
    .associate { columns ->
...
val verifyTableOwnershipRules by tasks.registering {
  group = "verification"
  description = "Verifies durable table access ownership defined by ADR-0106."
